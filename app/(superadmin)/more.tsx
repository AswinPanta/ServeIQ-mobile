...
const MORE_SECTIONS = [
  ...
  },
  {
    title: 'System',
    items: [
      { label: 'Health', icon: 'verified' as const, route: '/(superadmin)/system/health', color: '#EF4444' },
      { label: 'Audit Logs', icon: 'analytics' as const, route: '/(superadmin)/system/audit-logs', color: '#6366F1' },
      { label: 'Impersonate', icon: 'person.fill' as const, route: '/(superadmin)/system/impersonate', color: '#8B5CF6' },
    ],
  },
  {
    title: 'Admin',
    items: [
      { label: 'Roles & Permissions', icon: 'lock' as const, route: '/(superadmin)/admin/roles', color: '#10B981' },
      { label: 'Settings', icon: 'settings' as const, route: '/(superadmin)/admin/settings', color: '#6B7280' },
    ],
  },
];
    ...
    >
      ...
      <View style={styles.profileCard}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.profileEmail}>{admin?.email || 'admin@stayeasy.com'}</Text>
        </View>
            ...
            <Stagger step={50} initialDelay={100 + sIdx * 80} portal="superadmin">
              {section.items.map((item, i) => (
                ...
                >
                  ...
                  <IconSymbol name="arrow.forward" size={14} color="#CBD5E1" />
                </AnimatedPressable>
              ))}
            </Stagger>
          </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 13, fontWeight: '700', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10, marginLeft: 4 },
  sectionCard: {
    borderRadius: 16,
    backgroundColor: '#FFF',
    borderWidth: 1,
