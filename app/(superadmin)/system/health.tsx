...
const SERVICES = [
  ...
  { name: 'File Storage', status: 'Degraded', uptime: '98.50%', response: '320ms', lastIncident: '2025-06-28', color: AMBER[500] },
  { name: 'Email Service', status: 'Operational', uptime: '99.88%', response: '210ms', lastIncident: '2025-06-20', color: STATUS.activeGreen },
...
const STATUS_STYLES: Record<string, { bg: string; text: string }> = {
  Operational: { bg: EMERALD[500] + '12', text: STATUS.activeGreen },
  Degraded: { bg: AMBER[500] + '12', text: AMBER[500] },
        ...
        <View style={s.incidentCard}>
          ...
          {RECENT_INCIDENTS.map((inc, i) => (
            <View key={inc.title} style={[s.incidentRow, i < RECENT_INCIDENTS.length - 1 && { borderBottomWidth: 1, borderBottomColor: SLATE[100] }]}>
              ...
              <View style={[s.incidentStatus, { backgroundColor: EMERALD[500] + '12' }]}>
                <Text style={{ fontSize: 11, fontWeight: '700', color: STATUS.activeGreen }}>{inc.status}</Text>
              </View>
            </View>
          ))}
        </View>
...
const s = StyleSheet.create({
  ...
  statusBanner: { padding: 18, borderRadius: 16, backgroundColor: BG.white, borderWidth: 1, borderColor: SLATE[100], gap: 8 },
  statusDot: { width: 14, height: 14, borderRadius: 7 },
  bannerTitle: { fontSize: 17, fontWeight: '700', color: SLATE[900] },
  bannerDesc: { fontSize: 13, color: SLATE[500] },
  card: { padding: 16, borderRadius: 14, backgroundColor: BG.white, borderWidth: 1, borderColor: SLATE[100] },
  cardHead: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 },
  dot: { width: 10, height: 10, borderRadius: 5 },
  svcName: { fontSize: 15, fontWeight: '700', color: SLATE[900], flex: 1 },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 6 },
  statusText: { fontSize: 11, fontWeight: '700' },
  metricsRow: { flexDirection: 'row', gap: 16 },
  metricLabel: { fontSize: 11, color: SLATE[500] },
  metricValue: { fontSize: 14, fontWeight: '700', color: SLATE[900], marginTop: 2 },
  incidentCard: { padding: 18, borderRadius: 16, backgroundColor: BG.white, borderWidth: 1, borderColor: SLATE[100] },
  ...
  incidentTitle: { fontSize: 14, fontWeight: '600', color: SLATE[900] },
  incidentMeta: { fontSize: 12, color: SLATE[500], marginTop: 2 },
