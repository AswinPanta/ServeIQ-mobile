...
import { GREEN, RED, AMBER, CYAN, SLATE, BG, STATUS_COLORS, WARM, TEXT, FLAT } from '@/lib/constants/figma-tokens';
;
;
...
export default function TableOrderScreen() {
  ...
  if (!table) {
    return (
      <View style={{ flex: 1, backgroundColor: SLATE[50], justifyContent: 'center', alignItems: 'center', padding: 24 }}>
        <Text style={{ fontSize: 18, fontWeight: '600', color: RED[500] }}>Table not found</Text>
        <TouchableOpacity onPress={() => safeGoBack()} style={{ marginTop: 12, paddingHorizontal: 20, paddingVertical: 10, backgroundColor: ACCENT, borderRadius: 10 }}>
          <Text style={{ color: BG.white, fontWeight: '600' }}>Go Back</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={{ flex: 1, backgroundColor: SLATE[50] }}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        <View style={{ paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <View>
                <Text style={{ fontSize: 20, fontWeight: '700', color: SLATE[800] }}>Table {table?.number ?? ''}</Text>
                <Text style={{ fontSize: 13, color: SLATE[500] }}>Capacity: {table?.capacity ?? 0} · {table?.shape ?? ''}</Text>
              </View>
        ...
        <View style={{ paddingHorizontal: 16, gap: 16 }}>
          ...
          {showReadyBanner && (
            <View style={{ padding: 14, borderRadius: 14, backgroundColor: AMBER[500] + '20', borderWidth: 1, borderColor: AMBER[600] }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ fontSize: 13, fontWeight: '700', color: AMBER[700], flex: 1 }}>🍽️ ORDER READY — {readyItems.count} items ready for pickup!</Text>
                <TouchableOpacity onPress={() => setDismissedReadyAtCount(readyItems.count)}
                  style={{ paddingHorizontal: 12, paddingVertical: 4, borderRadius: 6, backgroundColor: AMBER[600] }}
                >
                  <Text style={{ fontSize: 11, fontWeight: '700', color: BG.white }}>Dismiss</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
          {isHappyHour && (
            <View style={{ padding: 14, borderRadius: 14, backgroundColor: FLAT.gold + '20', borderWidth: 1, borderColor: STATUS_COLORS.gold }}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: WARM.bronze }}>🎉 Happy Hour! 20% off selected items</Text>
            </View>
          )}
          {existingTickets.length > 0 && (
            <View style={{ padding: 14, borderRadius: 14, backgroundColor: AMBER[500] + '15', borderWidth: 1, borderColor: AMBER[500] + '30' }}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: AMBER[600] }}>⚠ {existingTickets.length} order(s) already in kitchen for this table</Text>
            </View>
          )}
          ...
          {cart.length > 0 && (
            <View style={{ padding: 16, borderRadius: 16, backgroundColor: BG.white, shadowColor: TEXT.black, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <Text style={{ fontSize: 15, fontWeight: '700', color: SLATE[800] }}>Current Order</Text>
                <Text style={{ fontSize: 13, fontWeight: '600', color: ACCENT }}>{cart.length} {cart.length === 1 ? 'item' : 'items'}</Text>
              </View>
              {cart.map((item, idx) => (
                <View key={`${item.menu_item_id}-${idx}`} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 6, borderBottomWidth: idx < cart.length - 1 ? 1 : 0, borderBottomColor: SLATE[100] }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 14, fontWeight: '600', color: SLATE[800] }}>{item.name}</Text>
                    {item.modifiers ? <Text style={{ fontSize: 11, color: SLATE[400], marginTop: 1 }}>{item.modifiers}</Text> : null}
                    <Text style={{ fontSize: 12, color: SLATE[500] }}>₹{item.unit_price} ea</Text>
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <TouchableOpacity onPress={() => { useOrderStore.getState().updateCartQty(tableId, item.menu_item_id, item.quantity - 1); }}
                      style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: RED[500] + '15', alignItems: 'center', justifyContent: 'center' }}
                    >
                      <Text style={{ fontSize: 14, fontWeight: '700', color: RED[500] }}>−</Text>
                    </TouchableOpacity>
                    <Text style={{ fontSize: 14, fontWeight: '700', color: SLATE[800], width: 20, textAlign: 'center' }}>{item.quantity}</Text>
                    <TouchableOpacity onPress={() => { useOrderStore.getState().updateCartQty(tableId, item.menu_item_id, item.quantity + 1); }}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: SLATE[100] }}>
                <Text style={{ fontSize: 15, fontWeight: '700', color: SLATE[800] }}>Total</Text>
          ...
          <View>
            <Text style={{ fontSize: 15, fontWeight: '700', color: SLATE[800], marginBottom: 10 }}>Menu</Text>
                ...
                {CATEGORIES.map((cat) => (
                  <TouchableOpacity key={cat} onPress={() => setActiveCategory(cat)}
                    style={{
                      ...
                      backgroundColor: activeCategory === cat ? ACCENT : BG.white,
                      borderWidth: 1, borderColor: activeCategory === cat ? ACCENT : SLATE[200],
                    }}
                  >
                    <Text style={{ fontSize: 13, fontWeight: '600', color: activeCategory === cat ? BG.white : SLATE[600] }}>{cat}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>
                ...
                return (
                  <TouchableOpacity key={item.id}
                    ...
                    style={{
                      ...
                      backgroundColor: BG.white, borderWidth: 1, borderColor: cartItem ? getAccentColor(0.3) : SLATE[100],
                      opacity: item.is_available ? 1 : 0.4,
                    }}
                    activeOpacity={0.8}
                  >
                    <View style={{ width: 10, height: 10, borderRadius: 5, backgroundColor: item.is_veg ? GREEN[500] : RED[500], marginRight: 10 }} />
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 14, fontWeight: '600', color: SLATE[800] }}>{item.name}</Text>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 }}>
                        {isHH ? (
                          <>
                            <Text style={{ fontSize: 13, fontWeight: '700', color: GREEN[500] }}>₹{hhPrice}</Text>
                            <Text style={{ fontSize: 11, color: SLATE[400], textDecorationLine: 'line-through' }}>₹{item.price}</Text>
                          </>
                        ) : (
                    ...
                    {!item.is_available && (
                      <View style={{ marginRight: 8, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, backgroundColor: RED[500] + '15', borderWidth: 1, borderColor: RED[500] + '40' }}>
                        <Text style={{ fontSize: 10, fontWeight: '700', color: RED[500] }}>OUT OF STOCK</Text>
                      </View>
                    )}
                    ...
                    {item.is_available && (
                      <View style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: ACCENT, alignItems: 'center', justifyContent: 'center' }}>
                        <Text style={{ color: BG.white, fontSize: 18, fontWeight: '700', lineHeight: 20 }}>+</Text>
                      </View>
                    )}
                  </TouchableOpacity>
                );
          ...
          {cart.length > 0 && (
            <TextInput
              ...
              style={{
                fontSize: 14, color: SLATE[800], paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12,
                backgroundColor: BG.white, borderWidth: 1, borderColor: SLATE[200],
              }}
            />
          )}
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            <TouchableOpacity onPress={handlePlaceOrder}
              style={{
                ...
                backgroundColor: existingTickets.length > 0 ? AMBER[600] : ACCENT,
                shadowColor: existingTickets.length > 0 ? AMBER[600] : ACCENT, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4,
              }}
              activeOpacity={0.85}
            >
              <Text style={{ fontSize: 15, fontWeight: '700', color: BG.white }}>{existingTickets.length > 0 ? 'Send as Supplement' : 'Place Order'}</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={handleSplitBill}
              style={{
                flex: 1, paddingVertical: 16, borderRadius: 16, alignItems: 'center',
                backgroundColor: BG.white, borderWidth: 1.5, borderColor: getAccentColor(0.3),
              }}
              activeOpacity={0.85}
            >
      ...
      <Modal
        ...
        actions={
          ...
          >
            <Text style={{ color: BG.white, fontWeight: '700', fontSize: 14 }}>Done</Text>
          </TouchableOpacity>
        }
      >
        {modifierModal?.item?.modifiers?.map((mod) => (
          <View key={mod.id} style={{ marginBottom: 16 }}>
            <Text style={{ fontSize: 14, fontWeight: '600', color: SLATE[800], marginBottom: 8 }}>{mod.name}</Text>
                ...
                return (
                  <TouchableOpacity key={opt.label} onPress={() => setSelectedMods((prev) => ({ ...prev, [mod.id]: opt.label }))}
                    style={{
                      ...
                      backgroundColor: isSelected ? ACCENT : SLATE[100],
                      borderWidth: 1, borderColor: isSelected ? ACCENT : SLATE[200],
                    }}
                  >
                    <Text style={{ fontSize: 13, fontWeight: '600', color: isSelected ? BG.white : SLATE[600] }}>
