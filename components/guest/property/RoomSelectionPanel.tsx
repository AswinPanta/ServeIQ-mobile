          ...
          return (
            <TouchableOpacity
              ...
              style={[
                s.card,
                {
                  borderColor: selectedRoom?.id === room.id ? ACCENT : SLATE[200],
                  backgroundColor: selectedRoom?.id === room.id ? 'rgba(46,134,171,0.04)' : BG.white,
            ...
            >
              ...
              <View style={[
                s.radio,
                {
                  borderColor: selectedRoom?.id === room.id ? ACCENT : SLATE[300],
                  backgroundColor: selectedRoom?.id === room.id ? ACCENT : 'transparent',
                },
              ]}>
                {selectedRoom?.id === room.id && <IconSymbol name="check" size={10} color={BG.white} />}
              </View>
            </TouchableOpacity>
          );
        })}
...
const s = StyleSheet.create({
  title: { fontSize: 14, fontWeight: '700', color: BRAND.navyLight, marginBottom: 10, letterSpacing: -0.2, fontFamily: FONTS.sora },
  warningBox: {
    padding: 12, borderRadius: 10, backgroundColor: 'rgba(211, 84, 0, 0.08)',
    borderWidth: 1, borderColor: 'rgba(211, 84, 0, 0.2)', marginBottom: 12,
  },
  warningText: { fontSize: 11, color: SRS.orange, fontWeight: '500', lineHeight: 16 },
  list: { gap: 12 },
  card: { flexDirection: 'row', gap: 12, padding: 12, borderRadius: 14, borderWidth: 1.5 },
  cardImg: { width: 72, height: 72, borderRadius: 10 },
  cardInfo: { flex: 1 },
  cardName: { fontSize: 13, fontWeight: '700', color: BRAND.navyLight },
  cardMeta: { fontSize: 11, color: SLATE[400], marginTop: 1 },
  disabledNote: { fontSize: 10, color: SRS.orange, fontWeight: '600', marginTop: 1 },
