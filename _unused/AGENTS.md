...
### 2026-08-06 — Quality Refactor: Tokenized Colors (Stage 1), `lib/api.ts` + `listing-wizard.tsx` + `booking-flow.tsx` Splits (Stage 2)

**Done (Stage 1 — color tokenization, fully verified)**
- Pass 1 (`scripts/tokenize-colors.mjs`) tokenized all 6/3-digit hex literals (~3,200 refs, 198 files) via a JSX-aware regex that also rewrote `style={{color: '#...'}}` → `style={{color: TOKEN}}` (inserted `={}`) and repaired dropped-equals `name{REF}`; merged duplicated token-import lines in 33 files (`scripts/merge-token-imports.mjs`)
- Pass 2 (`scripts/tokenize-alpha-colors.mjs`) tokenized 8-digit alpha hexes + grayscale shorthands (169 refs, 33 files) via `TOKEN + 'AA'` text-literal concatenation (base-8 mapping documented in the script); `#FFF`/`#000` shorthand → `BG.white`/`TEXT.black`
- `lib/constants/figma-tokens.ts` extended (new ramps + re-exports + `FLAT` ramp `{ blue, green, gold }`); backend hexes matched against literal values
- Fixed pass-2 side effects: 2 split multi-line imports repaired, `STATUS.warning/error` → `UI.warning/error` (2 files), explicit `string` annotations where literal inference broke (2 files)
- **Verification:** `npx tsc --noEmit` clean; `npm run lint` at baseline **44 errors / 132 warnings (all pre-existing)**; all codemods idempotent (re-run = 0 changes); 0 hexes remaining outside `lib/mock/**`/token files

**Done (Stage 2 — file splits)**
- `lib/api.ts` (large monolith) **deleted** → `lib/api/{index,client,mappers,properties,search,tenants}.ts`; all call-sites import `@/lib/api` unchanged; `getActiveToken` now internal-only
- `app/(host)/listing-wizard.tsx` (2,427 lines) → **712-line orchestrator** + `components/host/wizard/`:
  - `types.ts` — `WizardStep`/`PropertyData`/`LocationData`/`Offer` + `WizardCtx` (typed ctx passed to steps) + `PROPERTY_TYPES`/`COUNTRIES`/`AMENITY_OPTIONS`/`DEFAULT_OFFERS`/`STEP_ORDER`/`ACCENT`
  - `controls.tsx` — `CounterInput`/`ToggleSwitch`/`StarRating`/`ReviewCard`/`ReviewField` + their styles (`reviewCs`, `cs`)
  - `steps.tsx` — `ListingWizardSteps({ ctx })` default export; the 7 step renderers destructure ctx so JSX bodies stayed byte-identical
  - `styles.ts` — main `styles` StyleSheet (moved verbatim; `getAccentColor` lives only here)
- Extracted via `sed` line-range slicing (no JSX transcription) + hand-written wrappers; `export` keywords added to module-local declarations on extraction
- Lint improved to **44 errors / 130 warnings** (removed 2 dead imports: `API_BASE_URL` in listing-wizard, `AMENITY_OPTIONS` in steps)

**Done (Stage 2 — booking-flow split, fully verified)**
- `app/booking-flow.tsx` (1,254) → **thin composition screen** (~120 lines) +:
  - `components/booking/constants.ts` — `Step`/`SelectedRoom`/`GuestInfo`/`PaymentGateway` types + `PAYMENT_METHODS`/`GATEWAYS_WITH_SDK`/`IS_EXPO_GO`/`STRIPE_PUBLISHABLE_KEY`/`RAZORPAY_KEY_ID`/`KHALTI_PUBLIC_KEY`/`KHALTI_ENVIRONMENT`/`NAVY`/`BLUE`/`TEAL` + `calculateNights`/`formatDate`/`isPaymentVerified`/`gatewayUnavailableNote`
  - `components/booking/styles.ts` — shared `styles` StyleSheet (deduped `const styles = StyleSheet.create` opener)
  - `components/booking/steps.tsx` — plain named components (`BookingHeader`/`ProgressHeader`/`StepRooms`/`StepDetails`/`StepPayment`/`BottomBar`/`LoginGate`) — kills all 10 "component created during render"/"missing display name" errors
  - `hooks/use-booking-flow.ts` — `useBookingFlow()` hook: room fetch, selection, promo, totals, `handleNext`/`handleComplete` (WebView + native-SDK payment paths), guest-prefill effect, step-0-gate removed (gating lives in the screen)
- Screen renders `BookingHeader` + flow-driven steps; hosted checkout (`PaymentCheckoutModal`) + native SDK (`SdkPaymentCheckout`) rendered via `flow.checkout`/`flow.sdkCheckout`; modal completion/cancel handled by hook callbacks (`handleCheckoutComplete`/`handleCheckoutCancel`/`handleSdkComplete`/`handleSdkCancel`) that resolve the pending payment promise — screen never mutates refs (avoids react-compiler "This value cannot be modified")
- Verification: `npx tsc --noEmit` clean; lint improved to **32 errors / 123 warnings** (was 44/132 baseline: −12 errors, −9 warnings); remaining booking-file issues are pre-existing patterns relocated verbatim (guest-prefill + fetchRooms setState-in-effect, `fetchRooms` exhaustive-deps)

**Done (Stage 2 — host-context split)**
- `lib/context/host-context.tsx` (796) → **663-line provider** + `lib/context/host-utils.ts` (149) with the pure/stateless logic:
  - `SUBSCRIPTION_PLANS`/`SubscriptionPlan`/`checkPlanLimit` (SA-004 plan limits), `persistHostProperties` + storage keys (`HOST_PROPERTIES_KEY`/`HOST_DELETED_SEEDS_KEY`/`SEED_PROPERTY_IDS`), `OPS_DEFAULT_PROPERTY_ID/NAME_KEY`, `genId`
  - Backend→UI mappers extracted to plain exported functions: `mapApiProperty`, `mapApiRoom`, `mapApiStaff` — the two room/staff mappers that previously closed over `activePropertyId` now take it as a param (call sites updated: fetchHostData effect, rooms effect, refreshRooms, staff fetch, addStaff)
- External API unchanged: all 23 importers use `useHost` only; the named exports were internal-only (verified) so no re-exports needed
- Dropped 2 dead type imports (`ExtraCharge`, `CancellationPolicy`) + the unused `genId` from the provider; provider's direct eslint issues went **5 → 1** (only the pre-existing `fetchHostData` setState-in-effect); `npm run lint` totals unchanged (32/123) because `expo lint` only covers `/src`, `/app`, `/components` — `lib/` was never in that count

**Key decisions**
- Step components receive a single typed `ctx: WizardCtx` (state + setters + handlers + derived) and destructure at the top so the 800-line JSX bodies needed zero edits — minimal-diff, `tsc`-guaranteed
- `styles.ts` keeps its `getAccentColor(alpha)` calls (only consumer of the portal-theme helper)
- Payment gateway modals stay in the screen; the hook owns the resolver refs and exposes callback props instead of leaking refs through the return object (refs mutated via `flow.someRef.current` are invisible to react-compiler and error out)

**Next targets (remaining Stage 2)**
- Stage 4: `git mv` `AGENTS.md`/`docs/`/`scripts/`/etc. → `_unused/` + tsconfig/eslint excludes; final `tsc` + lint + 4-portal Expo Go smoke test

**Done (Stage 4 — artifact relocation, fully verified)**
- `git mv` `AGENTS.md`/`docs/`/`ServeIQ_SRS.md`/`scripts/` → `_unused/`; plain-moved untracked `build-report.sh` + `extract_nepal_outline.py` → `_unused/`; cleared stale `_unused/docs/` copy first (git rm) then moved live `docs/` in — all renames tracked (git history preserved, "R" status)
- `.env` **untracked** (`git rm --cached`) + `.env` added to `.gitignore` (was tracking real Stripe/Razorpay test keys + API URL)
- `tsconfig.json` `exclude` += `_unused`; `eslint.config.js` `ignores` += `_unused/**`
- Kept in place per plan: `.agents/`, `.claude/`, `.superpowers/`, `LICENSE`, `README.md`
- Repo root now reads as a clean app project (app/components/lib/hooks/constants/stores/types + config)
- Verification: `npx tsc --noEmit` clean; `npm run lint` **32 errors / 123 warnings** (unchanged — moved files are excluded); no app code references moved artifacts (grep clean); `CI=1 npx expo export --platform ios` **exit 0** (Metro resolves all modules post-refactor; web export timed out at 300s on static rendering but native bundle proves resolution)
- 4-portal Expo Go smoke test still **not run** (needs a device session)

**Next targets**
- Manual smoke test of all 4 portals in Expo Go (colors must render identically, booking flow rooms → details → payment)
- NOTE: `AGENTS.md` now lives in `_unused/AGENTS.md` (archived, not auto-loaded); open it directly for architecture/session context

### 2026-08-05 (later) — Native Payment SDKs: Khalti integrated, Stripe/Razorpay verified

**Done**
- Installed `expo-dev-client` (~57.0.10) + `@bishaldahal/react-native-khalti-checkout` (^0.2.2) via `npx expo install` (peer deps `*` — installs cleanly on Expo SDK 57 / RN 0.86)
- `app.json`: added `expo-dev-client` plugin; enabled `enableGooglePay` on the Stripe plugin; declared `ios.bundleIdentifier` + `android.package` (`com.ServeIQ.app`) — required for native builds
- `.env`: added `EXPO_PUBLIC_KHALTI_PUBLIC_KEY=` (empty — paste the key from the Khalti dashboard) and `EXPO_PUBLIC_KHALTI_ENVIRONMENT=TEST` (set `PROD` for live)
- `components/feature/sdk-payment-checkout.tsx`: added native **Khalti** gateway — `KhaltiPaymentSdk.startPayment({ publicKey, pidx, environment })` with success/error/cancel event listeners, lazy-required like Stripe/Razorpay; success reports the authoritative `pidx` (falls back to the intent pidx). Payload handling mirrors the installed SDK's real types (`build/KhaltiPaymentSdk.types.d.ts`): success = `{ pidx, status, ... }`, error = `{ error, details, ... }`; a resolved `status: 'cancelled'|'failed'` is never reported as success. Listener methods are null-guarded and a local `settle()` guard makes the first event (listener OR promise resolution OR error) win — a late event can never override an earlier one
- `app/booking-flow.tsx`: Khalti now prefers the native SDK in a development build (public key + pidx present); falls back to the existing hosted WebView checkout in Expo Go / web / no key. Stripe/Razorpay behavior unchanged. `sdkReady` also requires `!IS_EXPO_GO` so Expo Go always keeps the WebView fallback (never attempts a native SDK that can't run there)
- `npx tsc --noEmit` — zero errors
- `npx expo prebuild --no-install` — succeeded (stripe / expo-dev-client / splash plugins + razorpay/khalti autolinking all resolve)
- **Verified end-to-end (2026-08-05):** a real Khalti payment completed successfully through the native SDK flow (booking created → payment intent → pidx → native wallet checkout → backend confirm verified)

**Key decisions**
- Khalti stays dual-path: native SDK where it can run, WebView everywhere else — no regression in Expo Go or web. `sdkReady` explicitly requires `!IS_EXPO_GO` (otherwise a configured `EXPO_PUBLIC_KHALTI_PUBLIC_KEY` would wrongly route Expo Go into the native-SDK attempt instead of the working WebView fallback)
- `merchantIdentifier` NOT set on the Stripe plugin: it's only needed for Apple Pay and requires a real Apple merchant ID; `enableGooglePay` alone is safe. Add it when Apple Pay is enabled
- The native SDKs only run in a development build (`npx expo prebuild` + `npx expo run:ios|android`, or `eas build --profile development`) — Expo Go keeps working via fallbacks (WebView for Khalti, "switch method" note for Stripe/Razorpay)
- Expo SDK 57 default minSdk (24) ≥ Khalti's requirement (21) — no extra native config needed
- Store policy: hotel bookings are a physical/real-world service, so external payment processors (Stripe/Razorpay/Khalti) are permitted — no IAP requirement

---

### 2026-08-05 — Booking Flow: Timezone-safe Dates, Room Pre-selection Skip, Khalti return_url

**Problem context**
- The live backend 400s ("Check-in date cannot be in the past") because `Date#toISOString()` is UTC: in positive-offset timezones (Nepal +5:45) slicing it yields the previous local day. Backend requires exact `YYYY-MM-DD`.
- The reference web apps (Thadaw `thadaw-ref`, Samip `booking-ref`) have NO separate room-selection step — the detail page picks the room and booking starts at guest details.
- Live backend `anil-ref/app/utils/url_validation.py` hard-requires Khalti `return_url` to `startswith` its configured `KHALTI_RETURN_URL_BASE` (`http://localhost:5173/reserve`, the reference web app's `/reserve` page). The app's expo deep link fails this → `apiPost` falls back to a no-`pidx` object → confirm sends `{}` → "Payment Not Verified".

**Done**
- `constants/api-config.ts`: `toDateParam(value?)` now reconstructs the LOCAL date (`getFullYear`/`getMonth`/`getDate`) with regex passthrough for already-clean `YYYY-MM-DD`; added `KHALTI_RETURN_URL_BASE` (env-overridable via `EXPO_PUBLIC_KHALTI_RETURN_URL`, default `http://localhost:5173/reserve`).
- `app/booking-flow.tsx`: `checkIn`/`checkOut` normalized via `toDateParam` (replaces local `toDateOnly`); `returnUrlPrefix` now = `KHALTI_RETURN_URL_BASE` (used for both the Khalti intent `return_url` and the WebView completion-detection prefix); removed the now-unused `expo-linking` import.
- `components/feature/search-modal.tsx`: `toLocalDate` helper replaces `toISOString().split('T')[0]`.
- Hosted-checkout flow in place: `PaymentCheckoutModal` (`components/feature/payment-checkout-modal.tsx`, `react-native-webview`) opens the gateway `payment_url`, intercepts the return-URL redirect, extracts the gateway-returned `pidx`, and only then confirms — never before. Fallback "I've completed the payment" button for web/manual resume.
- Room pre-selection skip: `preselectMatched` auto-advances step 0→1 when the detail-page `roomId` matches a fetched room; progress header collapses to 2 steps; "Change room" / Back-to-step-0 reset it; room summary card at top of details step.
- `npx tsc --noEmit` — zero errors

**Key decisions**
- The Khalti `return_url` uses the backend's configured base (a dev-machine URL, meaningless on-device) purely to pass validation — the actual return is intercepted in the in-app WebView so no device browser / localhost web server is ever needed.
- Confirmation is driven by the gateway-returned `pidx` from the redirect (authoritative) over the intent-response `pidx`.
- `PaymentCheckoutModal` was added concurrently (webview + modal wiring already present); this session only corrected the `return_url` prefix that the backend rejected.
