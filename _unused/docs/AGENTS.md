...
- Source repo: `github.com/anilghatan6/Stay-Easy` (24 commits, main branch — as of 2026-07-31)
...
- `Samip-khatri/StayEasy-Booking` is a separate Vite+React frontend (not backend) — see Related Repos below
...
## Related Repos (tracked)

All four StayEasy repos — check for updates by comparing commit counts below against GitHub (as of 2026-07-31):

- **`anilghatan6/Stay-Easy`** — **backend source (FastAPI)** at `https://stay-easy-sizw.onrender.com`. 24 commits. Latest (Jul 31): search properties description; (Jul 30) booking created-at, room/bed type names, special offers in booking response, available-rooms router.
- **`Thadaw/StayEasy2.0`** — **active** Vite+React web frontend (`my-react-app/`). 27 commits. Latest (Jul 31): my-booking + booking details; (Jul 30) browse-by-property-type API; (Jul 29) Stripe; (Jul 28) Razorpay, booking confirmation, guest detail; (Jul 27) search/property/available-rooms API.
- **`Samip-khatri/StayEasy-booking`** — Vite+React frontend. 8 commits. Latest (Jul 30): dashboard fetching real-time property data.
- **`Thadaw/StayEasy`** — stale, 11 commits, last commit Jul 3, `/pms` route prefix. **Superseded by `Thadaw/StayEasy2.0`.**

## Mobile App
- **Expo SDK 57** with React Native
...
- `/payment/config` — public gateway client config (Stripe publishable key, Razorpay key id) — added 2026-07-31
...
- `github.com/anilghatan6/Stay-Easy` (now 24 commits; 19 at time of this analysis) is the **actual source repo** for the live backend at `https://stay-easy-sizw.onrender.com`
...
### 2026-07-31 — Real Stripe Checkout (guest booking flow)

**Gap analysis (vs Thadaw/StayEasy2.0 + Samip-khatri/StayEasy-booking)**
- Real Stripe SDK checkout was missing — local `booking-flow.tsx` had a hand-rolled card form that POSTed to the payment-intent endpoint but ignored `client_secret` and confirmed with no `gateway_payload`.
- Deferred (explicitly skipped): Nepal wallets (eSewa/Khalti/IME Pay/QR/Meal Card), printable receipt, live My Bookings, browse-by-property-type flow, pagination meta/nearby endpoints.

**Backend change (anilghatan6/Stay-Easy — MUST be pushed + redeployed)**
- New `app/modules/booking/routers/payment_config_router.py`: `GET /payment/config` → `{ stripe: { publishable_key }, razorpay: { key_id } }` from env.
- Requires `STRIPE_PUBLISHABLE_KEY` env var set on Render (paired with existing `STRIPE_SECRET_KEY`).
- Registered in `app/main.py` (`app.include_router(payment_config_router)`).

**App changes (Stay_Easy)**
- Added `@stripe/stripe-react-native` 0.64.0 + config plugin in `app.json` as `["@stripe/stripe-react-native", { "merchantIdentifier": "" }]` (bare-string form crashes the config plugin — reads `props.merchantIdentifier` on undefined).
- Native-only SDK; requires a dev build (`npx expo prebuild` / `run:ios`/`run:android`) — **does not work in Expo Go**.
- `lib/payment/stripe-sheet.ts` (native) + `stripe-sheet.web.ts` (stub returns `unsupported`) — keeps the web bundle from loading the native module.
- `lib/payment/payment-config.ts` — cached fetch of Stripe publishable key from `/payment/config`.
- `booking-flow.tsx` — on iOS/Android: createPaymentIntent → `initPaymentSheet(client_secret)` → `presentPaymentSheet()` → `confirmPayment({ gateway_payload: { payment_intent_id } })`. Canceled/failed/not-verified show alerts and keep the booking held. Web keeps the existing card form + demo fallback.
- `types/api.ts` + `constants/api-config.ts` + `lib/api/booking-api.ts`: added `PaymentClientConfig` and `getPaymentConfig`.

**Test infra fixes (pre-existing, unrelated to Stripe)**
- `jest.config.js` had a stray `)` syntax error in `transformIgnorePatterns`.
- `jest.setup.js` imported removed `@testing-library/react-native/extend-expect` and stubbed `react-native-worklets` with `{}` (broke reanimated mock → `createSerializable is not a function`). Added `resolver: react-native-worklets/jest/resolver.js`.
- `components/ui/__tests__/motion.test.tsx`: KpiCounter tests advanced fake timers only 8ms but display updates on a 32ms interval → bumped to 40ms.

**Verification (2026-07-31)**
- `npx tsc --noEmit` — clean on all touched files (repo has pre-existing errors elsewhere).
- `eslint` — 0 errors on changed files.
- `npx expo export --platform web` — succeeds (web stub resolution works).
- `npx jest` — 10/10 pass.
